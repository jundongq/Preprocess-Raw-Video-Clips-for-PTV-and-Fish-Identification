...
def frameExtractor(videofile, sample_frequency):
	...
	data_dir = data_dir + '/sampledFrames'
...
def check_run(videofile, sample_frequency):
	...
	if vidcap.isOpened():
		...
		t0 = time.time()
		frameExtractor(videofile, sample_frequency)
		t1 = time.time()
		...
		t0 = time.time()
		frameExtractor(videofile_converted, sample_frequency)
		t1 = time.time()
...
if __name__ == "__main__":
	print '~~~ Loading .MP4 file... & Sample Frequency ~~~'
	ap = argparse.ArgumentParser()
	ap.add_argument('-v', '--video', required=True, help='filename of one video clip')
	ap.add_argument('-n', '--frequency', type=int, required=True, help='sample frequency of the video frames')
	args = vars(ap.parse_args())

	videofile = args['video']
	sample_frequency = args['frequency']
	
	check_run(videofile, sample_frequency)
